        ...
        <div class="flex justify-between h-16">
            <div class="flex items-center space-x-8">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    <x-nav-link :href="route('explore_events')" :active="request()->routeIs('explore_events')" class="text-white hover:text-pink-500">
                        {{ __('Explore Events') }}
                    </x-nav-link>
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                @if(Auth::check())
                    <x-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            <button class="inline-flex items-center px-3 py-2 text-white bg-pink-300 hover:bg-pink-400 rounded-md">
                                {{ Auth::user()->name }}
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        <div class="pt-4 pb-1 border-t border-gray-200 dark:border-gray-600">
            <div class="mt-6 space-y-1">
                @if(Auth::check())
                    <x-responsive-nav-link :href="route('profile.edit')" class="text-pink-500 hover:text-white">
                        {{ __('Profile') }}
                    </x-responsive-nav-link>
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <x-responsive-nav-link :href="route('logout')"
                                               onclick="event.preventDefault(); this.closest('form').submit();" class="text-pink-500 hover:text-white">
                            {{ __('Log Out') }}
                        </x-responsive-nav-link>
                    </form>
                @else
